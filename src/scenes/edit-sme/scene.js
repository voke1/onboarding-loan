...
class EditSme extends React.Component {
  sme = new SME();
  ...
  render() {
    ...
    const { 
      ...
      registration_number, asset_category, employment_category, 
    } = sme;
    let { 
      trading, manufacturing, specialised_services, general_services 
    } = sme;
    ...
    try {
      manufacturing = JSON.parse(manufacturing)
    } catch {
      manufacturing = []
    }

    try {
      specialised_services = JSON.parse(specialised_services)
    } catch {
      specialised_services = []
    }

    try {
      general_services = JSON.parse(general_services)
    } catch {
      general_services = []
    }
    ...
    return (
      <BaseScene>
        ...
        <ScrollView contentContainerStyle={{
          ...
          />
          <DateInput
            defaultValue={date_year_of_starting_business}
          ...
          />
          <FormPicker
            defaultValue={business_premises}
          ...
          />
          <FormPicker
            defaultValue={employment_category}
